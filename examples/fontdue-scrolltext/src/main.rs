...
use graphics::sprite::{Vertex, Sprite};
use graphics::Align4;
...
fn psp_main() {
    ...
    let texture_buffer = allocator.alloc_texture_pixels((LEN*BUF_WIDTH) as u32, BUF_HEIGHT as u32, TexturePixelFormat::Psm8888);
    let texture_buffer = unsafe { slice::from_raw_parts_mut(texture_buffer.as_mut_ptr_direct_to_vram() as *mut u32, LEN as usize*BUF_WIDTH*BUF_HEIGHT) };
    let vertex_buffer = allocator.alloc_sized::<Vertex>(LEN as u32*2);
    let vertex_buffer = unsafe { slice::from_raw_parts_mut(vertex_buffer.as_mut_ptr_direct_to_vram() as *mut Align4<Vertex>, LEN*2) };
    ...
    layout.append(&[font.clone()], &fontdue::layout::TextStyle::new(TEXT, 60.0, 0));
    let x_positions = layout.glyphs().iter().map(|glyph| glyph.x as i32).collect::<Vec<i32>>();


    // Get character bitmaps, padded to nearest multiple of 4 pixel width, skipping whitespace
    let sprites: Vec<Option<(u32, Sprite)>> = TEXT.chars().enumerate().map(|(i, letter)|  {
        if !letter.is_whitespace() {
            ...
            for (k, alpha) in bitmap.iter().enumerate() {
                ...
                texture_buffer[j+i*BUF_WIDTH*BUF_HEIGHT] = 0x00ff_ffff | (*alpha as u32) << 24;
                j+=1;
            }

            let sprite = Sprite::new(FONT_COLOR, 0, 0, metrics.width as u32,  metrics.height as u32);
    ...
    loop {
        ...
        for i in 0..LEN {
            if let Some(mut sprite) = sprites[i] {
                sprite.1.set_pos(x_positions[i] + val as i32, 100 + (20.0 * unsafe { sinf(j as f32 + val / TAU) }) as i32);
                vertex_buffer[i*2..i*2+2].copy_from_slice(&sprite.1.as_vertices()); 
                graphics::draw_vertices(&vertex_buffer[i*2..i*2+2], &texture_buffer[i*BUF_WIDTH*BUF_HEIGHT..(i+1)*BUF_WIDTH*BUF_HEIGHT], sprite.0, BUF_WIDTH as u32, BUF_HEIGHT as u32, 1.0, 1.0);
            }
            j += 1;
        }
