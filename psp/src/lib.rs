...
#[cfg(feature = "std")]
#[doc(hidden)]
pub unsafe fn _set_cwd(cwd: *const u8, len: usize) {
    use std::os::psp::ffi::OsStrExt;
    let path = std::ffi::OsStr::from_bytes(core::slice::from_raw_parts(cwd, len));
    let _ = std::env::set_current_dir(path);
}
#[cfg(not(feature = "std"))]
#[doc(hidden)]
pub unsafe fn _set_cwd(cwd: *const u8, _len: usize) {
    sys::sceIoChdir(cwd);
}
            ...
            extern "C" fn module_start(argc_bytes: usize, argv: *mut c_void) -> isize {
                ...
                unsafe fn init_cwd(arg0: *mut u8) {
                    ...
                    if len > 0 {
                        ...
                        *arg0.add(len) = 0;
                        $crate::_set_cwd(arg0 as *const u8, len);
                        *arg0.add(len) = tmp;
                    }
                }
